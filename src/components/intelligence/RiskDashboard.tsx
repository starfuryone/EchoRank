...
// Grade colors one shade darker than the dark-canvas originals so they hold
// AA contrast on white (same Tailwind families: emerald/lime/amber/orange/rose).
const GRADE_COLOR: Record<string, string> = {
  A: '#10b981', B: '#65a30d', C: '#d97706', D: '#ea580c', F: '#e11d48',
};
...
function Gauge({ score, grade, t }: { score: number; grade: string; t: RiskDashboardCopy }) {
  ...
  return (
    <svg viewBox="0 0 120 108" className="w-full max-w-[260px]" role="img"
      ...
      <path d={arc(start, start + sweep, 46)} fill="none" stroke="#e5e7eb" strokeWidth="8" strokeLinecap="round" />
      {score > 0 && (
      ...
      <text x="60" y="60" textAnchor="middle" fill="#111827" fontSize="26" fontWeight="700"
      ...
      <text x="60" y="100" textAnchor="middle" fill="#9ca3af" fontSize="7"
...
function Sparkline({ history, t }: { history: { day: string; score: number }[]; t: RiskDashboardCopy }) {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="w-full" role="img" aria-label={t.sparklineAria}>
      ...
      <circle cx={xs[xs.length - 1]} cy={ys[ys.length - 1]} r="2.5" fill="#3b82f6" />
    </svg>
  );
}
...
function Delta({ label, value }: { label: string; value: number | null }) {
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2">
      <p className="text-[10px] uppercase tracking-wider text-gray-400">{label}</p>
      <p className={`font-mono text-sm font-semibold ${flat ? 'text-gray-500' : up ? 'text-red-600' : 'text-green-600'}`}>
        {flat ? '0' : `${up ? '▲ +' : '▼ '}${value}`}
      </p>
    </div>
  );
  ...
  return (
    <div className="mt-3 space-y-3 border-t border-gray-200 pt-3">
      <label className="block">
        <span className="text-[10px] uppercase tracking-wider text-gray-400">{t.revenueLabel}</span>
        <input type="number" min="0" step="100" value={revenue} inputMode="numeric"
          ...
          className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-2.5 py-1.5 font-mono text-sm text-gray-900 shadow-sm placeholder:text-gray-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500" />
      </label>
      <label className="block">
        <span className="text-[10px] uppercase tracking-wider text-gray-400">{t.emailsLabel}</span>
        <input type="text" value={emails} onChange={(e) => setEmails(e.target.value)}
          placeholder="you@company.com, ops@company.com"
          className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-2.5 py-1.5 text-sm text-gray-900 shadow-sm placeholder:text-gray-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500" />
      </label>
      <label className="flex items-center gap-2 text-xs text-gray-700">
        <input type="checkbox" checked={enabled} onChange={(e) => setEnabled(e.target.checked)}
          className="h-3.5 w-3.5 accent-blue-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500" />
        {t.alertsEnabled}
      </label>
      {error && <p className="text-xs text-red-600">{error}</p>}
      <button onClick={() => void save()} disabled={saving}
        className="rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-blue-700 disabled:opacity-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">
  ...
  if (empty) {
    return (
      <div className="rounded-xl border border-gray-200 bg-white p-10 text-center shadow-sm">
        <p className="font-mono text-4xl text-gray-400">—</p>
        <p className="mt-3 text-sm text-gray-700">{t.emptyTitle}</p>
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4 lg:grid-cols-3">
        {/* Gauge */}
        <section className="flex flex-col items-center rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <button onClick={() => void load(true)} disabled={state === 'loading'}
            className="mt-3 self-end rounded-lg border border-gray-300 bg-white px-3 py-1 text-xs text-gray-600 hover:bg-gray-50 hover:text-gray-900 disabled:opacity-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">
        ...
        <section className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <h2 className="text-[11px] font-semibold uppercase tracking-wider text-gray-400">{t.componentsTitle}</h2>
              ...
              return (
                <li key={key} title={hint}>
                  ...
                  <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-gray-100">
                    <div className="h-full rounded-full bg-blue-500" style={{ width: `${(v ?? 0) * 100}%` }} />
                  </div>
                </li>
              );
            })}
          ...
          <div className="mt-5">
            <h3 className="text-[11px] font-semibold uppercase tracking-wider text-gray-400">{t.trendTitle}</h3>
        ...
        {/* Drivers */}
        <section className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <h2 className="text-[11px] font-semibold uppercase tracking-wider text-gray-400">{t.driversTitle}</h2>
          {d.current.drivers.length === 0 ? (
            <p className="mt-4 text-xs text-gray-400">{t.driversEmpty}</p>
          ) : (
            <ul className="mt-3 divide-y divide-gray-100">
              {d.current.drivers.map((dr) => (
                <li key={dr.signalId} className="py-2.5">
                  ...
                  <p className="mt-1 line-clamp-2 text-xs text-gray-900">{dr.title}</p>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <section className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <h2 className="text-[11px] font-semibold uppercase tracking-wider text-gray-400">{t.revenueAtRiskTitle}</h2>
            <button onClick={() => setShowConfig((v) => !v)}
              className="rounded-lg border border-gray-300 bg-white px-2 py-0.5 text-[10px] text-gray-600 hover:bg-gray-50 hover:text-gray-900 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">
          ...
          {d.revenue ? (
            <div className="mt-3"
              ...
              <p className="font-mono text-3xl font-bold text-red-600">
                ...
                <span className="text-sm font-normal text-gray-400"> {t.perMonthAbbrev}</span>
              </p>
            ...
            <p className="mt-3 text-xs text-gray-400">{t.revenueUnset}</p>
          )}
        ...
        <section className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm lg:col-span-2">
          <h2 className="text-[11px] font-semibold uppercase tracking-wider text-gray-400">{t.alertsTitle}</h2>
          {alerts.length === 0 ? (
            <p className="mt-3 text-xs text-gray-400">{t.alertsEmpty}</p>
          ) : (
            <ul className="mt-3 divide-y divide-gray-100">
              {alerts.map((a) => (
                <li key={a.id} className="flex items-start gap-3 py-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-xs text-gray-900">{a.title}</p>
                    {a.body && <p className="mt-0.5 line-clamp-1 text-[11px] text-gray-400">{a.body}</p>}
                  </div>
      ...
      <p className="text-right font-mono text-[10px] text-gray-400">
