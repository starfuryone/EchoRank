...
const COMPONENT_LABELS: { key: keyof Components; label: string; hint: string }[] = [
  { key: 'negativePressure', label: 'Negative pressure', hint: 'Time-decayed weight of negative signals' },
  { key: 'velocity', label: 'Velocity', hint: 'This week vs 4-week baseline' },
  { key: 'criticalRecent', label: 'Criticals', hint: 'High-severity signals, last 30 days' },
  { key: 'visibility', label: 'AI visibility', hint: 'Inverse of latest audit score' },
  { key: 'stagnation', label: 'Signal coverage', hint: 'Data drought is a blind spot' },
];
...
function Gauge({ score, grade }: { score: number; grade: string }) {
  ...
  return (
    <svg viewBox="0 0 120 108" className="w-full max-w-[260px]" role="img"
      aria-label={`Reputation risk ${score} out of 100, grade ${grade}`}>
      ...
      {ticks.map((t, i) => {
        ...
        const [x2, y2] = polar(t, i % 3 === 0 ? 32 : 35);
      ...
      <text x="60" y="74" textAnchor="middle" fill={color} fontSize="10" fontWeight="600"
        fontFamily="ui-monospace, monospace" letterSpacing="1">GRADE {grade}</text>
      <text x="60" y="100" textAnchor="middle" fill="#71717a" fontSize="7"
        letterSpacing="1.5">REPUTATION RISK · 0–100</text>
    </svg>
  );
}

function Sparkline({ history }: { history: { day: string; score: number }[] }) {
  if (history.length < 2) {
    return <p className="text-xs text-zinc-500">History builds as daily snapshots accumulate.</p>;
  }
  const w = 260, h = 48;
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="w-full" role="img" aria-label="90-day risk trend">
...
export default function RiskDashboard() {
  ...
  const load = useCallback(async (refresh = false) => {
    setState('loading');
    try {
      const res = await fetch(`/api/reputation/risk${refresh ? '?refresh=1' : ''}`, {
  ...
  if (state === 'error') {
    return (
      <div className="rounded-lg border border-zinc-800 bg-zinc-900/60 p-8 text-center">
        <p className="text-sm text-zinc-300">Risk data didn't load.</p>
  ...
  if (empty) {
    return (
      <div className="rounded-lg border border-zinc-800 bg-zinc-900/60 p-10 text-center">
        ...
        <p className="mt-3 text-sm text-zinc-300">No signals ingested yet.</p>
    ...
    <div className="space-y-4">
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <section className="flex flex-col items-center rounded-lg border border-zinc-800 bg-zinc-900/60 p-5">
          <Gauge score={d.current.score} grade={d.current.grade} />
          <div className="mt-3 grid w-full grid-cols-2 gap-2">
            <Delta label="7-day" value={d.deltas.d7} />
            <Delta label="30-day" value={d.deltas.d30} />
          </div>
          <button onClick={() => void load(true)} disabled={state === 'loading'}
            ...
            {state === 'loading' ? 'Recomputing…' : 'Recompute now'}
          </button>
        </section>

        {/* Components */}
        <section className="rounded-lg border border-zinc-800 bg-zinc-900/60 p-5">
          <h2 className="text-[11px] font-semibold uppercase tracking-wider text-zinc-400">Risk components</h2>
          ...
          <div className="mt-5">
            <h3 className="text-[11px] font-semibold uppercase tracking-wider text-zinc-400">90-day trend</h3>
            <div className="mt-2"><Sparkline history={d.history} /></div>
          </div>
        </section>

        {/* Drivers */}
        <section className="rounded-lg border border-zinc-800 bg-zinc-900/60 p-5">
          <h2 className="text-[11px] font-semibold uppercase tracking-wider text-zinc-400">Top risk drivers</h2>
          {d.current.drivers.length === 0 ? (
            <p className="mt-4 text-xs text-zinc-500">No negative contributors in the window. Quiet is good.</p>
          ) : (
                ...
                <li key={dr.signalId} className="py-2.5">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="font-mono text-[10px] text-rose-400">
                      {Math.round(dr.contribution * 100)}% of pressure
                    </span>
                  </div>
                  <p className="mt-1 line-clamp-2 text-xs text-zinc-200">{dr.title}</p>
                  <p className="mt-0.5 font-mono text-[10px] text-zinc-500">
                    {new Date(dr.occurredAt).toLocaleDateString()} · sev {dr.severity.toFixed(2)}
                  </p>
                </li>
              ))}
            </ul>
      ...
      <p className="text-right font-mono text-[10px] text-zinc-600">
        {d.current.signalCount} signals in window · computed {new Date(d.computedAt).toLocaleTimeString()}
      </p>
    </div>
  );
}
